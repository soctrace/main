...
describe("CampaignRoute", () => {
  beforeEach(() => { auth.session = null; auth.loading = false; auth.bypassAuth = false; auth.signOut.mockReset(); accessMock.mockReset(); });
  it("redirects unauthenticated users and preserves pathname, query, and hash", () => {
    renderRoute();
    expect(screen.getByTestId("return-location")).toHaveTextContent("/campaigns/mijas-2027");
    expect(screen.getByTestId("return-location")).toHaveTextContent("?edition=client");
    expect(screen.getByTestId("return-location")).toHaveTextContent("#opportunity-map");
  });
  ...
  it("does not expose an unknown campaign configuration", async () => {
    auth.session = { user: { id: "member-id" } }; renderRoute("/campaigns/unknown-2027");
    await waitFor(() => expect(screen.getByText("Acceso restringido")).toBeVisible()); expect(accessMock).not.toHaveBeenCalled();
  });
});
