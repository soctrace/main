...
export function ElectionTimelinePrint(){
 const ordered=[...data.electionInventory].sort((a,b)=>a.election_date.localeCompare(b.election_date));
 return <section className="print-election-timeline" aria-labelledby="print-election-timeline-title"><header><p>Trayectoria electoral</p><h2 id="print-election-timeline-title">Doce elecciones. Distintas fotografías.</h2><span>Una cronología comparable de ganador municipal, cuota, participación y margen.</span></header><div>{ordered.map(item=>{const result=summary(item.election_id),winner=partyPresentation(result.winner,{compact:true});return <article key={item.election_id}><time dateTime={item.election_date}>{shortDate.format(new Date(`${item.election_date}T12:00:00`))}</time><h3>{item.official_name}</h3><dl><div><dt>Ganador</dt><dd className={winner.className}>{winner.label}</dd></div><div><dt>Cuota</dt><dd>{decimal.format(result.winner_share_pct)} %</dd></div><div><dt>Participación</dt><dd>{decimal.format(result.participation_pct)} %</dd></div><div><dt>Margen</dt><dd>{decimal.format(result.margin_pct_points)} pp</dd></div></dl></article>})}</div></section>
}
