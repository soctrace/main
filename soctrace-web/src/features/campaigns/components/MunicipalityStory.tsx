...
function EvidenceMap({ mode }: { mode: "population" | "growth" }) {
  ...
  const populationRank = [...snapshot.sections].sort((a, b) => b.population - a.population).findIndex(({ id }) => id === selected.id) + 1;
  const populationTotal = snapshot.sections.reduce((sum, section) => sum + section.population, 0);
  const populationValue = Number.isFinite(selected.population) ? number.format(selected.population) : "Dato no disponible";
  const populationShare = Number.isFinite(selected.population) && populationTotal > 0 ? `${decimal.format(selected.population / populationTotal * 100)} % del total · puesto ${populationRank} de ${snapshot.sections.length}` : "Dato no disponible";
  const variationValue = Number.isFinite(selected.growthPercent) ? `${selected.growthPercent > 0 ? "+" : ""}${decimal.format(selected.growthPercent)} %` : "Dato no disponible";
  const variationDetail = Number.isFinite(selected.growthAbsolute) ? `${selected.growthClass === "growth" ? "Crecimiento" : selected.growthClass === "decline" ? "Descenso" : "Sin cambio"} · ${selected.growthAbsolute > 0 ? "+" : ""}${number.format(selected.growthAbsolute)} personas · 2023–2024` : "Dato no disponible";
  return <div className={`municipality-real-map municipality-real-map--${mode}`}>
    <div className="municipality-map-toolbar"><div><span>{mode === "population" ? "Población por sección" : "Variación de población"}</span><strong className="text-slate-300">{mode === "population" ? "2024 · personas" : "2023–2024 · %"}</strong></div><EvidenceTag>Datos verificados</EvidenceTag></div>
    <svg className="municipality-map-svg" viewBox="0 0 960 680" preserveAspectRatio="xMidYMid meet" role="group" aria-label={mode === "population" ? "Mapa de población por sección censal de Mijas en 2024" : "Mapa de variación de población por sección censal de Mijas entre 2023 y 2024"}>
    ...
    <aside className="municipality-selection" aria-live="polite" data-selected-section={selected.id}><p><MapPin className="h-4 w-4" /> {selected.label}</p><strong>{mode === "population" ? populationValue : variationValue}</strong><span>{mode === "population" ? populationShare : variationDetail}</span><a href="#municipality-sources">Ver evidencia</a></aside>
  </div>;
}
...
export function MunicipalityStory() {
  ...
  return <section id="municipality" className="municipality-story" aria-labelledby="municipality-title">
    <div className="municipality-premise"><div><EvidenceTag>Hecho verificado · 2021–2025</EvidenceTag><p>02 · El municipio</p><h2 id="municipality-title" className="text-slate-200">Mijas crece,<br />pero no crece igual<br />en todo su territorio.</h2><span>El crecimiento municipal está verificado. Las diferencias territoriales se muestran en el último periodo con secciones comparables.</span></div></div>
    <div className="municipality-map-story"><div className="municipality-map-sticky"><EvidenceMap mode={mapMode} /></div><div className="municipality-map-steps"><article ref={populationStep}><div className="municipality-map-panel"><p>La población no ocupa el territorio de forma uniforme.</p><h3 className="text-slate-200">La concentración tiene una geografía concreta.</h3><span>Las cinco secciones con más población reúnen {number.format(snapshot.headline.topFivePopulation2024)} personas, el {decimal.format(snapshot.headline.topFiveSharePercent2024)} % del municipio en 2024.</span><EvidenceTag>Observación en 37 secciones</EvidenceTag></div></article><article ref={growthStep}><div className="municipality-map-panel"><p>El mismo territorio, otra lectura.</p><h3 className="text-slate-200">El crecimiento reciente también es desigual.</h3><span>Entre 2023 y 2024, {snapshot.headline.growingSections2023To2024} secciones crecieron y {snapshot.headline.decliningSections2023To2024} redujeron población. No se infieren causas.</span><EvidenceTag>Periodo comparable · 2023–2024</EvidenceTag></div></article></div></div>
    <div className="municipality-realities"><header><p>Cuatro observaciones territoriales</p><h3 className="text-slate-300">Un municipio.<br />Realidades distintas.</h3><span>Observaciones descriptivas, todavía no segmentos de campaña.</span></header><div className="municipality-observation-list">
      <article><span>01</span><h4>La población se concentra</h4><p><strong>{decimal.format(snapshot.headline.topFiveSharePercent2024)} %</strong> de la población reside en las cinco secciones más pobladas.</p></article>
      <article><span>02</span><h4>La densidad cambia radicalmente</h4><p><strong>33–63.533</strong> personas por km² entre los extremos seccionales.</p></article>
      <article><span>03</span><h4>El crecimiento no es uniforme</h4><p><strong>{snapshot.headline.growingSections2023To2024} / {snapshot.headline.decliningSections2023To2024}</strong> secciones con crecimiento / descenso.</p></article>
      <article><span>04</span><h4>La estructura de edad contrasta</h4><p><strong>{decimal.format(snapshot.headline.minimumAverageAge2024)}–{decimal.format(snapshot.headline.maximumAverageAge2024)}</strong> años de edad media entre los extremos seccionales.</p></article>
    </div></div>
  </section>;
}
