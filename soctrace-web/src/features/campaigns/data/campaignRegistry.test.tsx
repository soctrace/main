...
describe("Mijas campaign configuration", () => {
  ...
  it("loads a complete, uniquely-addressable commercial report", () => {
    expect(campaign).not.toBeNull(); expect(campaign?.chapters).toHaveLength(9);
    expect(new Set(campaign?.chapters.map(({ id }) => id)).size).toBe(9);
  ...
  it("grouped navigation references only configured chapters", () => {
    ...
    const links = screen.getAllByRole("link"); expect(links).toHaveLength(campaign!.chapters.length);
